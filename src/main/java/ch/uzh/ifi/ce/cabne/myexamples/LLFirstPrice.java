...
public class LLFirstPrice {
	...
	public static void main(String[] args) throws InterruptedException, IOException {
        ...
        // choose best response calculator for outer loop -> verification step
		...
		* ***************************/
		...
		// set sampler
		context.setSampler(new FirstPriceLLGSampler(context));  // FPLLGSampler uses BidIterator TODO: See what has to be changed for LL bids
		...
		bneAlgo.makeBidderSymmetric(1, 0); // TODO: verify second player strategy to bid his exact valuation
		...
		BNEAlgorithmCallback<Double, Double> callback = (iteration, type, strategies, epsilon) -> {
            ...
            for (int i=ngridpoints/2; i<=ngridpoints; i++) {  // second half of ngridpoints
                double v = strategies.get(2).getMaxValue() * i / ngridpoints;
                builder.append(String.format("%5.4f",v));
                builder.append(" ");
            ...
            // make the strategy monotone, so it can be inverted for importance sampling TODO: Check what exactly is going on in this loop!
            for (int i=0; i<3; i+=2) {
