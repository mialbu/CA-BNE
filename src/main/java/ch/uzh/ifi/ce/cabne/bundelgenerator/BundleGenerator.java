...
public class BundleGenerator {
    ...
    private final int overbiddingKey = 6;  // If more than 6 bidders then this must be adjusted.
        ...
        for (int iter = nrPlayers; iter > 0; iter--) {
            ...
            for (ArrayList<Integer> currentComb : lengthMap.get(iter)) {
                if (statusMap.get(currentComb) == Status.notProcessed) {
                    // test if feasible | if not, set processed/if feasible,
                    // set maximal and set all from its lattice to processed
                    if (checkFeasible(currentComb)) {
     ...
     */
    // checks for each maxfeasalloc if overbidder is in current alloc, if yes -> make new array, remove overbidder,
    // add overbidderkey (for bundlehashmap), then check if this is feasible, if yes add it to overbiddignmaxfeasallocs
    ...
    private static void processDownwardsLattice(ArrayList<Integer> comb, HashMap<ArrayList<Integer>, Status> map) {
        ...
        for (String i : lower) {
            ...
            for (int playersInAlloc = 0; playersInAlloc < sum; playersInAlloc++) {
                // iterate 0 to sum-1 - get i.charAt(iterate) -> if 1 then add the iterate-th nr in the comb_code to the currentBundle
                if (Integer.parseInt(String.valueOf(i.charAt(incr))) == 1) {
